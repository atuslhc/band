...
#if (AA00_PATCH==1)
static uint8 sensorDataLen = 0; //irTempLen. the length of sensorData used.
uint8 irTempDataWrite[IRTEMPERATURE_WRITE_BUFF_LEN]; //the AA00:AA02 command store buffer.
uint8 irTempLenWrite;  //the irTempDataWrite been filled.
#endif
...
bStatus_t IRTemp_AddService(void)
{
  ...
  return GATTServApp_RegisterService( irTempAttrTbl,
  ...
  switch ( param )
  {
    case SENSOR_DATA:
      ...
      if (len <= SENSOR_DATA_LEN)
          ...
          sensorDataLen = SENSOR_DATA_LEN;
      VOID osal_memcpy( sensorData, value, sensorDataLen );
...
#if (AA00_PATCH==1)
      *pLen = sensorDataLen;
      VOID osal_memcpy( pValue, pAttr->pValue, sensorDataLen );
#else
                                           ...
                                           uint8 *pValue, uint8 len, uint16 offset, uint8 method )
{
  ...
  switch ( uuid )
  {
    ...
    case SENSOR_CONFIG_UUID:
      ...
      if ( status == SUCCESS )
      {
        if (len<=IRTEMPERATURE_WRITE_BUFF_LEN)
          irTempLenWrite = len;
        else
          irTempLenWrite = IRTEMPERATURE_WRITE_BUFF_LEN; //FIXME: will truncated.
        VOID osal_memcpy( irTempDataWrite, pValue, irTempLenWrite );
