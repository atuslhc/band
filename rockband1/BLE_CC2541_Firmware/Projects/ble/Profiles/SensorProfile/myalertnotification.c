/******************************************************************************
 ...
 @file  myalertnotification.c (heartrateservice.c)
...
gattCharCfg_t *myAlertMeasClientCharCfg;
...
gattAttribute_t myAlertAttrTbl[] =
{
	...
	// My Alert Measurement Declaration [1]
	{
	...
	// My Alert Measurement Client Characteristic Configuration [3]
	{
 ...
 * LOCAL FUNCTIONS
 */
static bStatus_t myAlert_ReadAttrCB(uint16 connHandle, gattAttribute_t *pAttr, 
 ...
 */
bStatus_t MyAlert_AddService( void )
{
  uint8 status;
  myAlertMeasClientCharCfg = (gattCharCfg_t *)osal_mem_alloc( sizeof(gattCharCfg_t) *
                                                                linkDBNumConns );
  if ( myAlertMeasClientCharCfg == NULL )
  {
...
bStatus_t MyAlert_MeasNotify(uint16 connHandle, attHandleValueNoti_t *pNoti)
{
  ...
  if (value & GATT_CLIENT_CFG_NOTIFY)
  {
    ...
    if ( pNoti->pValue != NULL )
    {
      status = GATT_Notification(connHandle, pNoti, FALSE);
      if ( status != SUCCESS )
        GATT_bm_free( (gattMsg_t *)pNoti, ATT_HANDLE_VALUE_NOTI );
    }
    else
  ...
  */
  if (!(value & GATT_CLIENT_CFG_NOTIFY))
    GATTServApp_WriteCharCfg(connHandle, myAlertMeasClientCharCfg, GATT_CLIENT_CFG_NOTIFY);
  pNoti->handle = myAlertAttrTbl[MYALERT_MEAS_VALUE_POS].handle;
  if ( pNoti->pValue != NULL )
  {
    status = GATT_Notification(connHandle, pNoti, FALSE);
