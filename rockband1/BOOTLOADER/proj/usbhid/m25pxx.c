...
/* Desc: Initialize ext flash chip. Power(PD5/3), SPI(USART0) interface.
 *       then read chip identifier.
 */
void M25Pxx_INIT(void)
{
	...
	GPIO_DriveModeSet(FLASH_PW_PORT, gpioDriveModeHigh);  //set drive 20mA
	FLASH_PW_ON();
    //IsFlashPowerOn = true;
//	vTaskDelay(1);  //in bootload mode without system task.
	...
	SysCtlDelay(5000); //make a delay time to make sure clock signal stable after init flash.
...
void GetFlashCapacity(unsigned char data)
{
	switch(data)
	{
		...
		case 0x37: // 8M bytes   MX25U6435E
			systemStatus.flashInfo.flashSize  = 0x800000; //8M
			systemStatus.flashInfo.blockSize  = 0x10000; //64k
			systemStatus.flashInfo.sectorSize = 0x1000;//4k
			systemStatus.flashInfo.sectors    = 0x800; //2048 sectors
			systemStatus.flashInfo.blocks     = 0x80; //128 blocks
			systemStatus.flashInfo.sectorsPerBlock = 0x10; //16 sector
...
/* Desc: Turn on ext flash power and setup SPI interface.
 *
 */
...
/* Desc: read flash address,length to calculate the CRC16
 * param: 
 *   address: the calculate start address.
 *   length: the total bytes will be calculated.
 * return:
 *   calculated crc16
 */
