...
osThreadId osThreadCreate(const osThreadDef_t *thread_def, void *argument) {
	...
	stackSize =
			thread_def->stacksize ?
					thread_def->stacksize / 4 : configMINIMAL_STACK_SIZE;
	xTaskCreate((pdTASK_CODE )thread_def->pthread,
			(const signed portCHAR *)thread_def->name, stackSize, argument,
...
{
	((osTimerDef_t*)timer_def)->arg = argument;
...
osMessageQId osMessageCreate(const osMessageQDef_t *queue_def,
	...
	cb = pvPortMalloc(sizeof(os_messageQ_cb_t));
	if(cb == NULL){
		return NULL;
	}
	cb->handle = xQueueCreate(queue_def->queue_sz, queue_def->item_sz);
	if(cb->handle == NULL){
		vPortFree(cb);
		return NULL;
	}
...
osStatus osMessagePut(osMessageQId cb, uint32_t info, uint32_t millisec) {
	portBASE_TYPE taskWoken = pdFALSE;
	portTickType ticks;
	...
	if (inHandlerMode()) {
		if (xQueueSendFromISR(cb->handle, (const void *)&info, &taskWoken)
				!= pdTRUE) {
			return osErrorOS;
		}
		portEND_SWITCHING_ISR(taskWoken);
	} else {
		if (xQueueSend(cb->handle, (const void *)&info, ticks) != pdTRUE) {
			return osErrorOS;
		}
	}
...
osEvent osMessageGet(osMessageQId cb, uint32_t millisec){
	portBASE_TYPE taskWoken;
	portTickType ticks;
	osEvent event;

	event.def.message_id = cb;
	...
	if (inHandlerMode()) {
		if (xQueueReceiveFromISR(cb->handle, &event.value.p, &taskWoken) == pdTRUE) {
			/* We have mail */
		...
		if (xQueueReceive(cb->handle, &event.value.p, ticks) == pdTRUE) {
			/* We have mail */
			event.status = osEventMessage;
		}
		else {
...
osMailQId osMailCreate (const osMailQDef_t *queue_def, osThreadId thread_id)
{
	...
	cb = pvPortMalloc(sizeof(struct os_mailQ_cb));
	if (cb == NULL) {
		return NULL;
	}
	cb->queue_def = queue_def;
	...
	cb->handle = xQueueCreate(queue_def->queue_sz, sizeof(void *));
	if (cb->handle == NULL) {
		vPortFree(cb);
		return NULL;
	}
	...
	/* Create a mail pool */
	cb->pool = osPoolCreate(&pool_def);
	if (cb->pool == NULL) {
		vQueueDelete(cb->handle);
		vPortFree(cb);
		return NULL;
	}
...
void osSemaphoreAddToRegistry(osSemaphoreId s, signed char* name)
{
	vQueueAddToRegistry(s, name);
}

void osMsgQAddToRegistry(osMessageQId mq, signed char* name)
{
	vQueueAddToRegistry(mq->handle, name);
}

void osMailQAddToRegistry(osMailQId mq, signed char* name)
{
	vQueueAddToRegistry(mq->handle, name);
}
