...
CONST uint8 irTempDataUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(IRTEMPERATURE_DATA_UUID),HI_UINT16(IRTEMPERATURE_DATA_UUID),
};

// IR Temp Characteristic value Configuration UUID
CONST uint8 irTempConfUUID[ATT_BT_UUID_SIZE] =
{
...
static uint8 irTempDataUserDesp[] = "DataUpLoder1.Data";
...
static uint8 irTempCfgUserDesp[] = "DataUpLoder1.Conf";

static uint8 irTempLen = 0;
uint8 irTempDataWrite[20];
uint8 irTempLenWrite;
/*********************************************************************
...
bStatus_t IRTemp_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
    case IRTEMPERATURE_DATA:
      ...
      {
        irTempLen = len;
        VOID osal_memcpy( irTempData, value, irTempLen );
                                           ...
                                           uint8 *pValue, uint8 len, uint16 offset )
{
  ...
  switch ( uuid )
  {
    case IRTEMPERATURE_DATA_UUID:
      ...
      //在这里就是接收到来自手机端APP的数据，把这些数据写到内存中，然后再通过串口发送给watch的MCU。
    case IRTEMPERATURE_CONF_UUID:
      // Validate the value
