...
{
  HDL_ANCS_NOTI_CCCD,                     // Discover ANCS NOTI CCCD
  HDL_ANCS_DATA_CCCD,                     // Discover ANCS DATA CCCD
};
...
uint8 timeAppConfigNext( uint8 state )
{
 ...
 uint8 ucDataArry[6]={UART_DATA_START,sizeof(ucDataArry),UART_CMD_ANCS,0,0,UART_DATA_STOP};
  ...
  switch ( timeAppConfigList[state] )
  {
    ...
    case HDL_ANCS_DATA_CCCD: 
      
      DEBUG_PRINT("Data enable#\r\n",0);
      ucDataArry[4]=2;
  ...
  if( timeAppHdlCache[HDL_ANCS_DATA_CCCD]==0)DEBUG_PRINT("No Data Handle!!!\r\n",0);
      break;
    case HDL_ANCS_NOTI_CCCD:    
      ucDataArry[4]=1;
      DEBUG_PRINT("Notify enable#\r\n",0);
      read = FALSE;
      writeReq.len = 2;
      writeReq.value[0] = LO_UINT16(GATT_CLIENT_CFG_NOTIFY);
      writeReq.value[1] = HI_UINT16(GATT_CLIENT_CFG_NOTIFY);
  ...
  if ( read )
  {
    ...
    GATT_ReadCharValue( gapConnHandle, &readReq, sensorTag_TaskID );
  ...
  else
  {
    ...
    uint8 ucStatus =GATT_WriteCharValue( gapConnHandle, &writeReq, sensorTag_TaskID );
...
{
  gattMsgEvent_t Msg;
  osal_memcpy( &Msg, pMsg, sizeof(gattMsgEvent_t) );
  
  if ( state > HDL_CACHE_LEN )
  {
    return TIMEAPP_CONFIG_CMPL;
  }
