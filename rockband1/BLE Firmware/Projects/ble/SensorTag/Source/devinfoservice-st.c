...
static uint8 devInfoModelNumberUserDesp[] = "Model.";
#endif
#include "SensorTag.h"
...
uint8 devInfoSerialNumber[] ="FF FF FF FF FF FF";


#if (SERVICE_DESCRIBE_OPEN==TRUE)
// Heart Rate Measurement Characteristic User Description
static uint8 devInfoSerialNumberUserDesp[] = "Serial Num.";
#endif

// Software Revision String characteristic
static uint8 devInfoSoftwareRevProps = GATT_PROP_READ;
uint8 devInfoSoftwareRev[] = " 0.00";
#if (SERVICE_DESCRIBE_OPEN==TRUE)
// Heart Rate Measurement Characteristic User Description
static uint8 devInfoSoftwareRevUserDesp[] = "Software Ver.";
#endif


// Firmware Revision String characteristic
static uint8 devInfoFirmwareRevProps = GATT_PROP_READ;
static uint8 devInfoFirmwareRev[] = " 1.20 ("__DATE__")";
#if (SERVICE_DESCRIBE_OPEN==TRUE)
...
#if (SERVICE_DESCRIBE_OPEN==TRUE)
// Characteristic User Description
static uint8 devInfoMfrNameUserDesp[] = "Manufacturer.";
#endif

extern void DevInfoRequestCB(void);
  

//Œ Ã‚£∫’‚¿Ô√ÊªÒ»°…Ë±∏–≈œ¢∫Û «‘ı—˘∞—’‚–©–≈œ¢∏¯ ÷ª˙∂ÀµƒAPPµƒ£ø
void DevInfoInit(void)
{
  
  //MAC
  devInfoSerialNumber[0]=HEX_TO_ASCII_H(mac_buf[5]);
  devInfoSerialNumber[1]=HEX_TO_ASCII_L(mac_buf[5]);

  devInfoSerialNumber[3]=HEX_TO_ASCII_H(mac_buf[4]);
  devInfoSerialNumber[4]=HEX_TO_ASCII_L(mac_buf[4]);
 
  devInfoSerialNumber[6]=HEX_TO_ASCII_H(mac_buf[3]);
  devInfoSerialNumber[7]=HEX_TO_ASCII_L(mac_buf[3]);  
  
  devInfoSerialNumber[9]=HEX_TO_ASCII_H(mac_buf[2]);
  devInfoSerialNumber[10]=HEX_TO_ASCII_L(mac_buf[2]);  
   
  devInfoSerialNumber[12]=HEX_TO_ASCII_H(mac_buf[1]);
  devInfoSerialNumber[13]=HEX_TO_ASCII_L(mac_buf[1]);  
  
  devInfoSerialNumber[15]=HEX_TO_ASCII_H(mac_buf[0]);
  devInfoSerialNumber[16]=HEX_TO_ASCII_L(mac_buf[0]); 

  
#if Dev_FW_V1<10
  devInfoFirmwareRev[0]=' ';
  devInfoFirmwareRev[1]=Dev_FW_V1+'0';
#else
  devInfoFirmwareRev[0]=Dev_FW_V1/10+'0';
  devInfoFirmwareRev[1]=Dev_FW_V1%10+'0';
#endif
  
#if Dev_FW_V2<10
  devInfoFirmwareRev[3]=' ';
  devInfoFirmwareRev[4]=Dev_FW_V2+'0';
#else
  devInfoFirmwareRev[3]=Dev_FW_V2/10+'0';
  devInfoFirmwareRev[4]=Dev_FW_V2%10+'0';
#endif  
  DevInfoRequestCB();
