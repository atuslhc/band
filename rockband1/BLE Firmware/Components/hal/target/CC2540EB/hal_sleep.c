...
void halSleep( uint32 osal_timeout )
{
  ...
  // max allowed sleep time in ms
  if (osal_timeout > MAX_SLEEP_TIMEOUT)//osal_timeoutÏµÍ³Ë¯ÃßÊ±¼ä¡£
  {
    osal_timeout = MAX_SLEEP_TIMEOUT;
  }

  // get LL timeout value already converted to 32kHz ticks
  LL_TimeToNextRfEvent( &sleepTimer, &llTimeout );//llTimeout
  ...
  if (osal_timeout == 0)//Èç¹ûÃ»ÓÐÏµÍ³³¬Ê±
  {
    // use common variable
    timeout = llTimeout;
  ...
  if ( (timeout > PM_MIN_SLEEP_TIME) || (timeout == 0) )
  {
   ...
   //   HalLedEnterSleep();
      ...
      P1 ^= 0x01;
      //P1 |= 0x01;
      extern void uartRxIsrInit(void);
      uartRxIsrInit();
      HAL_SLEEP_SET_POWER_MODE();//Ë¯ÃßÊÇ²»»áÔËÐÐµÄ£¬Ö±µ½Íâ²¿ÖÐ¶Ï¿ªÆô¡£
      extern void uartRxInit(void);
      uartRxInit();
      P1 &= ~0x01;
#ifdef DEBUG_GPIO
      // TEMP
