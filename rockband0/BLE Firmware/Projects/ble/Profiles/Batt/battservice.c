...
static uint8 BattLevelUserDesp[] = "BattLevel";
#endif
/*********************************************************************
...
// Battery level characteristic
static uint8 battLevelProps = GATT_PROP_READ;//GATT_PROP_READ | GATT_PROP_NOTIFY;
...
static uint8 battLevelData[20];
static uint8 battLevelLen;
/*********************************************************************
...
bStatus_t Batt_SetParameter( uint8 param, uint8 len, void* value )
{
	...
	switch ( param )
	{
		case BATT_PARAM_CRITICAL_LEVEL:
			...
			battLevelLen = len;
			VOID osal_memcpy( battLevelData, value, len );
...
bStatus_t Batt_MeasLevel( void )
{
	...
	if (level < battLevel)
	{
		...
		// Send a notification
		battNotifyLevel(); // send a battery characteristic notification while conecting.
	}
 ...
 */
uint8 ucLevel = 100;
void BattPeriodicityFill(uint8 ucValue)
{
	battLevel = ucValue;
}
static uint8 battReadAttrCB( uint16 connHandle, gattAttribute_t* pAttr,
                             uint8* pValue, uint8* pLen, uint16 offset, uint8 maxLen )
{
	...
	if ( uuid == BATT_LEVEL_UUID )
	{
		...
		*pLen = 1;
		pValue[0] = battLevel;
		VOID osal_memcpy( &pValue[0], battLevelData, battLevelLen );
	}
                                  ...
                                  uint8* pValue, uint8 len, uint16 offset )
{
	...
	switch ( uuid )
	{
		case GATT_CLIENT_CHAR_CFG_UUID:  //write attribe - general is for setting.
...
static void battNotifyCB( linkDBItem_t* pLinkItem )
{
	if ( pLinkItem->stateFlags & LINK_CONNECTED )
	{
		...
		if ( value & GATT_CLIENT_CFG_NOTIFY )
		{
			...
			noti.len = battLevelLen;
			VOID osal_memcpy( &noti.value[0], battLevelData, battLevelLen );

			//noti.value[0] = battLevel;


			GATT_Notification( pLinkItem->connectionHandle, &noti, FALSE );//set a characteristic notify value to a client.
		}
	}
}
#endif
...
static uint8 battMeasure( void )
{
	...
	HalAdcSetReference( HAL_ADC_REF_125V );//Atus: Why voltage still need ADC? It can be get from MCU.
 ...
 * @return      none
 */
//Apply while the battservice connction change.
