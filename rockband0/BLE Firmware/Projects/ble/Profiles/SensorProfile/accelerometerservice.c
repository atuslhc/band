...
CONST uint8 accelServUUID[ATT_BT_UUID_SIZE] =
{
	LO_UINT16(ACCELEROMETER_SERV_UUID), HI_UINT16(ACCELEROMETER_SERV_UUID)
};

// Accelerometer Characteristic value Data UUID
CONST uint8 accelDataUUID[ATT_BT_UUID_SIZE] =
{
	LO_UINT16(ACCELEROMETER_DATA_UUID), HI_UINT16(ACCELEROMETER_DATA_UUID)
};

// Accelerometer Characteristic value Configuration UUID
CONST uint8 accelCfgUUID[ATT_BT_UUID_SIZE] =
{
	LO_UINT16(ACCELEROMETER_CONF_UUID), HI_UINT16(ACCELEROMETER_CONF_UUID)
};

// Accelerometer Characteristic value Configuration UUID
CONST uint8 accelPeriodUUID[ATT_BT_UUID_SIZE] =
{
	LO_UINT16(ACCELEROMETER_PERI_UUID), HI_UINT16(ACCELEROMETER_PERI_UUID)
};

#if 0
//新增加的 heartrate 属性   仅仅做测试。
CONST uint8 HearAlertUUID[ATT_BT_UUID_SIZE] =
{
	LO_UINT16(HEART_ALERT_CHAR_UUID), HI_UINT16(HEART_ALERT_CHAR_UUID)
};
#endif
...
static uint8 accelDataLen = 1;
/*********************************************************************
 * Profile Attributes - variables
 */

// Accelerometer  Profile Service attribute
static CONST gattAttrType_t accelService = { ATT_BT_UUID_SIZE, accelServUUID };
...
static uint8 accelDataUserDesp[] = "ACK.Data";
...
static uint8 accelCfgUserDesp[] = "ACK.Conf";
...
static uint8 accPerUserDesp[] = "ACK.Period";


#if 0
//为了查错(使用升级工具不能下载数据)，先屏蔽 2015年8月12日11:43:18
//----------------新加的 2015-7-8 11:44:29------------------------
static uint8  heartAlertProps = GATT_PROP_NOTIFY;
static uint8  heartAlertData  = 150;

static uint8 heartAlertDesp[] = "Notification.Alert";

static gattCharCfg_t heartAlertClientCharCfg[GATT_MAX_NUM_CONN];

static accelerometerServiceCB_t accelerometerServiceCB;
...
gattAttribute_t accelAttrTbl[] =
{
	...
	// Characteristic Value "Data" 2
	{
		{ ATT_BT_UUID_SIZE, accelDataUUID },   //UUID aa11
		GATT_PERMIT_READ,
		0,
		accelData
	},
	...
	/********************每个UUID都需要先声明，服务包含的数据（如果有的话），配置，描述(但描述可以省略)*******************/
	...
	{
		{ ATT_BT_UUID_SIZE, accelCfgUUID },      //uuid shi aa12
		GATT_PERMIT_READ | GATT_PERMIT_WRITE,
		0,
		&accelCfg
	},
	...
	{
		{ ATT_BT_UUID_SIZE, accelPeriodUUID },  //uuid aa13
		GATT_PERMIT_READ | GATT_PERMIT_WRITE,
		0,
		&accPer
	},
...
//--------------Heart Alert ----------------------------------------
	...
	// heart rate alert
	{
		{ ATT_BT_UUID_SIZE, HearAlertUUID},
		0,
		0,
		&heartAlertData
	},
...
bStatus_t Accel_AddService( uint32 services )
{
	...
	GATTServApp_InitCharCfg( INVALID_CONNHANDLE, heartAlertClientCharCfg );//这是新加的 2015年7月9日10:51:39
#endif
...
bStatus_t Accel_GetParameter( uint8 param, void* value )
{
	...
	{
		case ACCELEROMETER_DATA:
			VOID osal_memcpy( value, accelData, ACCELEROMETER_DATA_LEN ); //把accelData中的数据拷贝到value中。
			break;
                             ...
                             uint8* pValue, uint8* pLen, uint16 offset, uint8 maxLen )
{
	...
	switch ( uuid )
	{
		...
		case ACCELEROMETER_DATA_UUID://把数据读取了。
			*pLen = accelDataLen;
			VOID osal_memcpy( pValue, pAttr->pValue, accelDataLen );
			break;
                                  ...
                                  uint8* pValue, uint8 len, uint16 offset )
{
	...
	switch ( uuid )
	{
		...
		case GATT_CLIENT_CHAR_CFG_UUID:
			...
			if (pAttr->handle == accelAttrTbl[13].handle) //只处理notification报警的，并且是配置的那个UUID
			{
				// heart rate alter cfg

				if ( status == SUCCESS )
				{
					uint16 charCfg = BUILD_UINT16( pValue[0], pValue[1] );

					(*accelerometerServiceCB)( (charCfg == GATT_CFG_NO_OPERATION) ?
					                       HEARTRATE_ALERT_NOTI_DISABLED :
					                      HEARTRATE_ALERT_NOTI_ENABLED );
				}
			}
#endif
			break;
	...
	{
		accel_AppCBs->pfnAccelChange( notifyApp );//这里就调用那2个属性值。
	}
...
static void acc_HandleConnStatusCB( uint16 connHandle, uint8 changeType )
{
	...
	if ( connHandle != LOOPBACK_CONNHANDLE )
	{
		          ...
		          ( !linkDB_Up( connHandle ) ) ) )
		{
			...
			GATTServApp_InitCharCfg( connHandle, heartAlertClientCharCfg );//这是新加的 2015年7月9日10:51:39
#endif
		}
	}
}
...
// Position of heart rate alert value in attribute array
#define HEARTRATE_ALERT_VALUE_POS            12

bStatus_t acc_HeartAlertNotify( uint16 connHandle, attHandleValueNoti_t* pNoti )
{
	uint16 value = GATTServApp_ReadCharCfg( connHandle, heartAlertClientCharCfg );

	// If notifications enabled
	if ( value & GATT_CLIENT_CFG_NOTIFY )
	{
		// Set the handle
		pNoti->handle = accelAttrTbl[HEARTRATE_ALERT_VALUE_POS].handle;

		// Send the notification
		return GATT_Notification( connHandle, pNoti, FALSE );//该函数表示server需要给client发送一个特征值的通知，不需要确认。
	}
 ...
 * @fn      ACC_Register
 *
 * @brief   Register a callback function with the ACC Service.
 *
 * @param   pfnServiceCB - Callback function.
 *
 * @return  None.
 */
extern void Acceler_Register( accelerometerServiceCB_t pfnServiceCB )
{
	accelerometerServiceCB = pfnServiceCB;
}
