...
CONST uint8 testServUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(TEST_SERV_UUID), HI_UINT16(TEST_SERV_UUID)
};

// Data Characteristic UUID
CONST uint8 testDataUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(TEST_DATA_UUID), HI_UINT16(TEST_DATA_UUID)
};

// Config Characteristic UUID
CONST uint8 testConfUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(TEST_CONF_UUID), HI_UINT16(TEST_CONF_UUID)
};
...
static CONST gattAttrType_t testService = { ATT_BT_UUID_SIZE, testServUUID };
  ...
  switch ( uuid )
  {
    case TEST_CONF_UUID:
      ...
      if ( offset == 0 )
      {
        ...
        {
            extern void setCONN_INTERVAL(uint8 conn_interval);
            setCONN_INTERVAL(pValue[0]);
        }
      }
      else
      {
