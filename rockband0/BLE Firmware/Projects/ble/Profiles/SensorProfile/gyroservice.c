...
CONST uint8 gyroServUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(GYROSCOPE_SERV_UUID),HI_UINT16(GYROSCOPE_SERV_UUID)
};

// Gyroscope Characteristic value Data UUID
CONST uint8 gyroDataUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(GYROSCOPE_DATA_UUID),HI_UINT16(GYROSCOPE_DATA_UUID)
};

// Gyroscooe Characteristic value Configuration UUID
CONST uint8 gyroCfgUUID[ATT_BT_UUID_SIZE] =
{
  LO_UINT16(GYROSCOPE_CONF_UUID),HI_UINT16(GYROSCOPE_CONF_UUID)
};
...
static CONST gattAttrType_t gyroService = { ATT_BT_UUID_SIZE, gyroServUUID };

// Accelerometer Characteristic Properties
static uint8 gyroDataProps = GATT_PROP_WRITE_NO_RSP | GATT_PROP_WRITE | GATT_PROP_NOTIFY;//GATT_PROP_WRITE | GATT_PROP_NOTIFY;
...
static uint8 gyroDataUserDesp[] = "DataUpLoder2.Data";
...
static uint8 gyroCfgUserDesp[] = "DataUpLoder2.Conf";

static uint8 gyroDataLen = 1;

uint8 gyroTempDataWrite[20];
uint8 gyroTempLenWrite;
...
gattAttribute_t sensorGyroscopeAttrTbl[] =
{
  {
    ...
    // Characteristic Declaration
    {
      ...
      {
        { ATT_BT_UUID_SIZE, gyroDataUUID },// Here is AA51, use for upload gather data.
        GATT_PERMIT_WRITE,
        0,
        gyroData
      },
...
bStatus_t Gyro_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
    case GYROSCOPE_DATA:
      ...
      {
        gyroDataLen = len;
        VOID osal_memcpy( gyroData, value, gyroDataLen );
                            ...
                            uint8 *pValue, uint8 *pLen, uint16 offset, uint8 maxLen )
{
  ...
  switch ( uuid )
  {
    ...
    case GYROSCOPE_DATA_UUID:
      *pLen = gyroDataLen;
      VOID osal_memcpy( pValue, pAttr->pValue, gyroDataLen );
      break;
                                           ...
                                           uint8 *pValue, uint8 len, uint16 offset )
{
  ...
  switch ( uuid )
  {
    case GYROSCOPE_DATA_UUID:
      ...
      //Should not get here
      //len 是长度
      //uint8 buf = pValue[0];//取收到的数据

	  gyroTempLenWrite = len;
      VOID osal_memcpy( gyroTempDataWrite, pValue, gyroTempLenWrite );
	  notifyApp = GYROSCOPE_DATA;
