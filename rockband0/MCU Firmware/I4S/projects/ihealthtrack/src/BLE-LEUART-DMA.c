...
void ParseBleUartPak(void)
{
	...
	for(int i = 0; i < RX_BUF_SIZE; i++)
	{
		...
		Rec_Ble_Pak[len++] = TempChar;//Rec_Ble_Pak中存储的是除过帧头/尾，长度后的内容。
	}

	//
	systemStatus.iBleHeartBeatCounter = 0;

	//Atus: Should check the len==0 and not found STOP command.
